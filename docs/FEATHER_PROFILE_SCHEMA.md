...
7. **PC patient hint (β.59+):** Hosts re-send `client_info` with `pc_user` on patient change; phone soft-matches `profile_id` / `display_name` / slug; Tech Keep/Switch when Feather/Simulate would override (PROTOCOL §8.2). Name profiles like host patients (or matching ids) so soft-match works — factory “Patient 1” will not match “Payton” until renamed.
