...
**Phone:** **v1.0.0-beta.75**. Discover `features` includes `ritual_list` and `ritual_unavailable`.  
...
- Capture panel, one block per stored recording, newest first. Line 1: local time, RMSSD, duration. Line 2: patient name and `sent` or `pending`. **Send** and **Delete** links are on the right of that block. Delete asks for confirmation.
