...
**Status:** Shipping phone contract through **v1.0.0-beta.49**; host light passes + FT bench/caregiver path recorded. **FlareTracker Bridge Companion** is live (**v1.0.6**). **Priority:** ECG-Box **Tuner** Online GET / Send / Store field-verify — see [`TUNER_AGENT_HANDOFF.md`](https://github.com/JoelAtHome/ecg-box/blob/main/docs/TUNER_AGENT_HANDOFF.md). β.49: ECG sensor modal action row wrap fix. β.48: Capture Stream/Record host hints; Simulate Feather blocked modal; ECG sensor actions row. β.47: Tech chrome simplify. β.46: source-picker chrome polish + Feather contact honesty. β.45: unified Polar/Feather source picker. β.44: NDJSON `coeffs_get` → `mcu_coeffs` (Online GET).
