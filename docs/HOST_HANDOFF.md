...
**Status:** Shipping phone contract through **v1.0.0-beta.41**; host light passes + FT bench/caregiver path recorded. **FlareTracker Bridge Companion** is live (**v1.0.6**). **Priority:** ECG-Box **Tuner** USB Apply + profile sync field-verify — see [`TUNER_AGENT_HANDOFF.md`](https://github.com/JoelAtHome/ecg-box/blob/main/docs/TUNER_AGENT_HANDOFF.md). β.41: Tuner re-push on Tech profile edits; [`UI_LAYOUT_MAP.md`](./UI_LAYOUT_MAP.md). β.40: ECG `peak_flags` + Tuner live stream/coeff push. **Parked:** phone UI chrome polish (unified H10/Feather connect CTAs) — mark up the layout map.
