...
**Status:** Shipping phone contract through **v1.0.0-beta.43**; host light passes + FT bench/caregiver path recorded. **FlareTracker Bridge Companion** is live (**v1.0.6**). **Priority:** ECG-Box **Tuner** Live Send / Store / Offline echo field-verify — see [`TUNER_AGENT_HANDOFF.md`](https://github.com/JoelAtHome/ecg-box/blob/main/docs/TUNER_AGENT_HANDOFF.md). β.43: Tech Offline Get/Store/Send; `offline_echo` while Tuner linked (phone Offline locked). β.42: `coeffs_push`/`coeffs_ack`; `profile_put` SoR only. **Parked:** phone UI chrome polish (unified H10/Feather connect CTAs) — mark up [`UI_LAYOUT_MAP.md`](./UI_LAYOUT_MAP.md).
