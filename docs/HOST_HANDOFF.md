...
**Status:** Shipping phone contract through **v1.0.0-beta.40**; host light passes + FT bench/caregiver path recorded. **FlareTracker Bridge Companion** is live (**v1.0.6**). **Priority:** ECG-Box **Tuner** USB Apply + profile sync field-verify — see [`TUNER_AGENT_HANDOFF.md`](https://github.com/JoelAtHome/ecg-box/blob/main/docs/TUNER_AGENT_HANDOFF.md). β.40: ECG `peak_flags` + Tuner live stream/coeff push. β.39: dirty-gated Saves + Tuner profile_put merge. β.38: PolarH10Bridge → `phone-bridge` / `ECG-Phone-Bridge` APK + Tuner profile sync wire. **Parked:** phone UI chrome polish (unified H10/Feather connect CTAs).
