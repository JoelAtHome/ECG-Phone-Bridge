...
- [ ] **Feather lead-off:** on `status` with `use_leads_off` + `leads_off`, show “check electrodes” / pause coaching when `use_leads_off && leads_off` (PROTOCOL §3.1). Do **not** invent contact from ECG SNR; do **not** reuse Polar `sensor_quality` for LOD.  
...
- [ ] **Feather lead-off:** when `status` includes `use_leads_off` + `leads_off`, show “check electrodes” if both true (PROTOCOL §3.1). Ignore when `use_leads_off` is false. Not `sensor_quality`.  
