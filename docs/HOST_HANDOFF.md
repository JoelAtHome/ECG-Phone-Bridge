...
**Status:** Shipping phone contract through **v1.0.0-beta.42**; host light passes + FT bench/caregiver path recorded. **FlareTracker Bridge Companion** is live (**v1.0.6**). **Priority:** ECG-Box **Tuner** Live Send / Store field-verify — see [`TUNER_AGENT_HANDOFF.md`](https://github.com/JoelAtHome/ecg-box/blob/main/docs/TUNER_AGENT_HANDOFF.md). β.42: NDJSON `coeffs_push`/`coeffs_ack` (Send → Feather only); `profile_put` is Store/SoR only. β.41: Tuner re-push on Tech profile edits; [`UI_LAYOUT_MAP.md`](./UI_LAYOUT_MAP.md). **Parked:** phone UI chrome polish (unified H10/Feather connect CTAs) — mark up the layout map.
